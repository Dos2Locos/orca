...
describe('GitHub GraphQL rate-limit guard', () => {
  ...
  // FORK PATCH (drop on the upstream release that fixes this): v1.4.180 cherry-picked
  // the 'adds and removes PR comment reactions through GraphQL' test onto the release
  // branch without setPRCommentReaction itself — the implementation exists only on
  // upstream main (client.ts, ipc/github.ts, runtime, preload, ChecksPanel). The test
  // body therefore does not compile, and `pnpm build` runs typecheck. Removed verbatim;
  // restore by taking upstream's side of this hunk once the release carries the function.
