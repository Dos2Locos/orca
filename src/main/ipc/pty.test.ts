...
describe('registerPtyHandlers', () => {
  ...
  beforeEach(() => {
    ...
    // Why kill fires onExit: the local provider now waits for a physical exit
    // before resolving a kill, so the default mock must behave like node-pty or
    // every kill in this suite would sit out that timeout.
  ...
  describe('spawn environment', () => {
    ...
    it('rechecks a restart exit against the replacement provider before cleanup', async () => {
      ...
      try {
        ...
        expect(runtime.onPtyExit).toHaveBeenCalledWith('restarted-claude-session', -1, undefined)
